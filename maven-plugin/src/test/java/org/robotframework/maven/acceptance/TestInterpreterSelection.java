...
public class TestInterpreterSelection extends TestCase {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        mojo = new RobotMojo();
        mojo.robotTestDirectory = new File(
                "src/test/resources/robot-tests/interpreter_test.html");
    }

    public void testRobotIsExecutedSuccessfullyWithJython()
            throws Exception {
        mojo.robotScript = RobotInstallation.pathToJybot();
        mojo.robotArguments.add("--variable");
        mojo.robotArguments.add("INTERPRETER:jython");
        mojo.execute(); // no exception should be thrown
    }

    public void testRobotIsExecutedSuccessfullyWithPython()
            throws Exception {
